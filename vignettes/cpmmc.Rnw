...
\end{table}
  ...
  As mentioned in Section 2.2, in order for there to be small relative variance for the likelihood ratio estimator $\frac{\hat{p}(y | \theta\prime, U\prime)}{\hat{p}(y | \theta, U)}$, the CPM algorithm requires $(\theta, U)$ to be "close" enough to $(\theta\prime, U\prime)$, and that $\hat{p}(y | \cdot, \cdot)$ is a regular enough function. However, it may not always be possible to fulfil the requirements, and \cite{cpmmDeligiannidis2015} had used the state-space model as an example, where the likelihood ratio estimator does not behave as required even for "close" enough $(\theta, U)$ and $(\theta\prime, U\prime)$. In this case, \cite{cpmmDeligiannidis2015} introduced a resampling scheme procedure based on the Hilbert space-filling curve and the "Particle filter using Hilbert sort" algorithm in order to sample from the state-space model. \\

The CPM algorithm can be computationally complex during implementations. In the case of the random effects model, the CPM scheme have to simulated $N$ samples to compute the likelihood estimation using importance sampling estimator for each observed data points, and this has to be repeated $T$ times to compute the acceptance probability for a single iteration. If the parameter of interest is of dimension $p$, then the CPM algorithm has to simulate $NTp$ random variables per iteration to get one sample, which can be inefficient if the size of the sample required is large. \\

In addition, the heuristic provided by \cite{cpmmDeligiannidis2015} to select parameters for CPM in order to optimise the performance of the algorithm is expensive to compute. In practice, the suggested procedure is to evaluate $CT(h, Q_{T})$ only on a subset of data, then the parameters can be estimated through regression.
