...
\centerline{$x \sim f_\theta(\cdot)$} \newline
\centerline{$Y_t| X_t) \sim g_\theta(\cdot | X_t)$} \\
...
If the integral for $p(y_t| \theta)$ is intractable, one can use importance sampling with $N$ correlated particles to form an unbiased estimator $\hat{p}(y_t| \theta)$ in the CPM algorithm. \\
